...
<?php include("inc/header.php"); ?>
				...
				<div class="row">
				 <div class="col-md-4 col-sm-offset-1">
                        ...
                        <div class="ribbons">
                            ...
                            <div class="ribbon ribbon-blue"><span>Welcome  <?php echo $_SESSION['firstname']; ?> to Pair Game
		</span>     </div>   
                            <!--/ Ribbons -->
                        </div>
						 ...
						 <div id="timebar"></div>
							...
							<form action="#" method="post" id="commentForm" class="ajax_form">
								...
								#################### games index : 1-wiriting 2-pics and words 3-COMPLETE THE SENTENCE 4-PAIR GAME 5-LISTEN and WRITE 6-translation  ####################
									$sql_qst="SELECT * FROM questions WHERE id_game=4 ORDER BY RAND() LIMIT 1";
  ...
  <input type="hidden" value="<?php echo $id_question; ?>" id="qst"/>
     ...
     <?php  	#####################Printin Question variable $question into this bloc ##################### ?>
  ...
  <div class="add-comment-title" > </div>
								...
								<div id="help_tag">
									...
									shuffle($array_cls);
									##################### While finding answer options in database write it in html format with bootstrap random style in styled checkboxes #################
		...
		<div class="[ form-group ]" >
            <input type="checkbox" name="answer_text" value="<?php echo $aff_answ['id']; ?>" id="fancy-checkbox-<?php echo $array_cls[$i]; ?>" autocomplete="off" />
            <div class="[ btn-group ]">
                <label for="fancy-checkbox-<?php echo $array_cls[$i]; ?>" class="[ btn btn-<?php echo $array_cls[$i]; ?> ]">
                    <span class="[ fa fa-check-square-o ]"></span>
                    <span> </span>
                </label>
                <label for="fancy-checkbox-<?php echo $array_cls[$i]; ?>" class="[ btn btn-default active ]" style="font-size:12px">
                     <?php echo $aff_answ['answer_text']; ?>
                </label>
            </div>
        </div>
...
jQuery(document).ready(function ($) {
	/* dsable other checkbox if two are checked*/
	$(document).on('change', ":checkbox[name='answer_text']", function(){
	  if ($(":checkbox[name='answer_text']:checked").length == 2) {
    ...
    $(function() {
		...
		/* radio */
		 ...
		 var chkArray = [];
		  ...
		  $("input[name='answer_text']:checked").each(function() {
        ...
        $.ajax({
            type: 'POST',
            url: 'services/check_answer4.php', 
            dataType: "json",
